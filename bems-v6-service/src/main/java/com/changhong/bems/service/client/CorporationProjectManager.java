...
public class CorporationProjectManager {
     ...
     * @return 分页查询结果
     */
    public ResultData<List<ProjectDto>> findByPage(String erpCorporationCode, String searchValue) {
        ...
        if (resultData.successful()) {
            ...
            for (CorporationProjectDto dto : dtoList) {
                if (StringUtils.isNotBlank(dto.getWbsProjectCode())) {
                    projectList.add(new ProjectDto(dto.getWbsProjectCode(), dto.getWbsProjectName()));
                } else if (StringUtils.isNotBlank(dto.getInnerOrderCode())) {
                    projectList.add(new ProjectDto(dto.getInnerOrderCode(), dto.getInnerOrderName()));
                }
            }
